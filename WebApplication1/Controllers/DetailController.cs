...
namespace WebApplication1.Controllers
{
    ...
    public class DetailController : Controller
    {
        ...
        // 2026.01.29 Changed: 보류(hold)를 결재 종료가 아닌 유보(PendingHold)로 처리하여 대기 목록에서 사라지지 않게 하고, 동일 n차 결재자만 추후 승인/반려 재처리 가능하도록 수정
        ...
        public async Task<IActionResult> ApproveOrHold([FromBody] ApproveDto dto)
        {
            ...
            // ===== (2) 공통 판정: 보류 계열(Status) 통일 =====
            static bool __IsHoldLikeStatus(string? st)
            {
                if (string.IsNullOrWhiteSpace(st)) return false;
                st = st.Trim();

                // DocumentApprovals.Status: PendingHold, PendingHoldA1 등
...
WHERE DocId = @DocId
  AND ISNULL(Status,N'Pending') LIKE N'Pending%';";
                        ...
                        {
                            Value = _helper.GetCurrentUserDisplayNameStrict() ?? string.Empty
                        });
...
WHERE DocId = @DocId AND StepOrder = @Step;";
                    ...
                    // DocumentApprovals 또는 Documents 둘 중 하나라도 보류계열이면 "이미 보류"
                    if (__IsHoldLikeStatus(apprStatus) || __IsHoldLikeStatus(docStatus))
                    {
                        // 신규 리소스키를 임의 생성하지 않기 위해 기존 키로 처리
...
WHERE DocId = @id AND StepOrder = @step;";
                ...
                if (actionLower == "approve")
                {
                    ...
                    else
                    {
                        ...
                        try
                        {
                            ...
                            await DocControllerHelper.SendApprovalPendingBadgeAsync(notifier: _webPushNotifier, S: _S, conn: conn, targetUserIds: nextApproverUserIds ?? new List<string>(), url: "/", tag: "badge-approval-pending");
                        }
                ...
                {
                    // 보류/반려는 작성자에게: "해당 문서 URL" + "보류/반려 캡션" (다국어 리소스 사용, 특정 언어 하드코딩 금지)
                    try
                    {
                            ...
                            && !string.Equals(authorId, approverId, StringComparison.OrdinalIgnoreCase))
                        {
                            ...
                            var bodyText = (actionLower == "hold")
                                ...
                                : (_S?["PUSH_ApprovalReject"] ?? "PUSH_ApprovalHold").ToString();
                            ...
                            await _webPushNotifier.SendToUserIdAsync(
                                ...
                                title: titleText,
                                body: bodyText,
                                url: "/Doc/Detail?id=" + Uri.EscapeDataString(dto.docId), // 해당 문서 URL 전달
                                tag: tag
                            );
                        }
                    }
...
WHERE a.DocId = @DocId
  ...
  AND ISNULL(a.Status, N'Pending') LIKE N'Pending%';";
            ...
            // Reader 중에는 추가 쿼리 금지 → ApproverValue만 모아둠
            ...
            await using (var r = await cmd.ExecuteReaderAsync())
            {
                while (await r.ReadAsync())
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(av))
                        approverValuesToResolve.Add(av.Trim());
                }
            }

            // Reader 종료 후 Resolve + Backfill
            var toBackfill = new List<(string approverValue, string resolvedUserId)>();
            foreach (var av in approverValuesToResolve
                ...
                .Distinct(StringComparer.OrdinalIgnoreCase))
            {
                ...
                {
                    var rid = resolved.Trim();
                    list.Add(rid);
                    toBackfill.Add((approverValue: av, resolvedUserId: rid));
                }
            }

            if (toBackfill.Count > 0)
            {
                // CS0411 원인: (tuple key + comparer) 조합으로 GroupBy 타입 추론 실패
                // 해결: 대소문자 무시가 필요한 문자열 결합 키로 DistinctBy 처리
                foreach (var x in toBackfill
                    .Select(t => new
                    {
                        approverValue = (t.approverValue ?? "").Trim(),
                        resolvedUserId = (t.resolvedUserId ?? "").Trim(),
                        key = ((t.approverValue ?? "").Trim()) + "\u001F" + ((t.resolvedUserId ?? "").Trim())
                    })
...
WHERE DocId = @DocId
  AND StepOrder = @Step
  AND (UserId IS NULL OR LTRIM(RTRIM(UserId)) = N'')
  AND ApproverValue = @ApproverValue;";
            ...
            while (await rdr.ReadAsync())
            {
                ...
                list.Add(new
                {
                    ...
                    actedAtText = actedAt.HasValue
                                           ? _helper.ToLocalStringFromUtc(actedAt.Value)
                                           : string.Empty,
...
WHERE UserId = @UserId;";
            ...
            if (createdAtUtc.HasValue)
            {
                ...
                createdAtText = _helper.ToLocalStringFromUtc(createdAtForUi.Value);
            }
...
ORDER BY a.ActedAt DESC;";
                ...
                if (obj != null && obj != DBNull.Value)
                {
                    ...
                    recalledAtText = _helper.ToLocalStringFromUtc(recalledAtForUi.Value);
                }
            }
            catch (Exception ex)
            {
...
ORDER BY UploadedAt, FileId ASC;";
                ...
                while (await fr.ReadAsync())
                {
                    ...
                    if (upUtc.HasValue)
                    {
                        ...
                        upText = _helper.ToLocalStringFromUtc(up);
                    }
...
ORDER BY v.ViewedAt DESC;";
            ...
            return View("~/Views/Doc/Detail.cshtml");
        }
...
WHERE s.DocId = @DocId
  ...
  AND (s.ExpireAt IS NULL OR s.ExpireAt > SYSUTCDATETIME());";
                ...
                sc.Parameters.Add(new SqlParameter("@UserId", SqlDbType.NVarChar, 450) { Value = (object)userId ?? DBNull.Value });
            ...
            {
                _log.LogWarning(ex, "LogDocumentViewAsync: check DocumentShares failed. DocId={DocId}, UserId={UserId}", docId, userId);
            }
...
VALUES
    (@DocId, @ViewerId, @ViewerRole, SYSUTCDATETIME(), @ClientIp, @UserAgent);";
            ...
            // 2) 공유 읽음 처리: tab=shared 이고 실제 공유 대상자일 때만
...
WHERE DocId = @DocId
  ...
  AND ISNULL(IsRead, 0) = 0;";
                ...
                {
                    _log.LogWarning(ex, "LogDocumentViewAsync: update DocumentShares.IsRead failed. DocId={DocId}, UserId={UserId}", docId, userId);
                }
            }
        }
...
WHERE d.DocId = @id;";
            ...
            {
                if (!System.IO.Path.IsPathRooted(resolvedOutput))
                {
                    resolvedOutput = resolvedOutput.TrimStart('\\', '/');
                    resolvedOutput = System.IO.Path.Combine(_env.ContentRootPath, resolvedOutput);
                }

                resolvedOutput = resolvedOutput.Replace('/', System.IO.Path.DirectorySeparatorChar)
                                               .Replace('\\', System.IO.Path.DirectorySeparatorChar);
            }
...
GROUP BY ParentCommentId;";
            ...
            // 기존: ORDER BY ThreadRootId, Depth, CreatedAt 로 인해
            // depth=1 전체가 먼저 출력되고 depth=2가 뒤로 밀려 "답글 위치가 이상"해 보일 수 있음
            // 변경: 재귀 CTE로 SortPath(부모/자식 경로) 생성 후 SortPath로 정렬 → 부모 다음에 자식이 항상 바로 붙음
            cmd.CommandText = @"
;WITH Tree AS
(
    SELECT
    ...
    SELECT
        ...
        ch.CreatedAt,
        ch.UpdatedAt,
        CAST(t.SortPath + N'/' + RIGHT(REPLICATE('0', 20) + CONVERT(varchar(20), ch.CommentId), 20) AS nvarchar(max)) AS SortPath
    FROM dbo.DocumentComments ch
    INNER JOIN Tree t
        ON t.CommentId = ch.ParentCommentId
    WHERE ch.DocId = @DocId
      AND ch.IsDeleted = 0
)
SELECT  t.CommentId,
        t.DocId,
...
LEFT JOIN dbo.AspNetUsers u
       ON  u.Id       = t.CreatedBy
       OR  u.UserName = t.CreatedBy
       OR  u.Email    = t.CreatedBy
...
OPTION (MAXRECURSION 100);";
            ...
            while (await rd.ReadAsync())
            {
                ...
                var createdAtLocal = _helper.ToLocalStringFromUtc(
        ...
        public async Task<IActionResult> PostComment([FromBody] DocCommentPostDto dto)
        {
            try
            {
                ...
                // 댓글 CreatedBy 저장값: UserId 우선 (없으면 Name 폴백)
                var currentUserId = User?.FindFirstValue(ClaimTypes.NameIdentifier) ?? "";
                var currentUserName = User?.Identity?.Name ?? "";
...
VALUES
    (@DocId, @ParentCommentId, @ThreadRootId, @Depth,
     ...
     @CreatedBy, SYSUTCDATETIME(), SYSUTCDATETIME());";
                ...
                // 최상위 댓글이면 ThreadRootId를 자기 자신으로 세팅
...
WHERE ParentCommentId = @id AND DocId = @docId AND IsDeleted = 0;";
            ...
            var childCount = Convert.ToInt32(childCountObj);
            if (childCount > 0)
            {
